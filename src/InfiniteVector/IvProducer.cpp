...
, reservePosition_(resolver_.resolve<volatile IvReservePosition>(header_->reservePosition_))
...
void IvProducer::publish(Buffers::Buffer & buffer)
{
    ...
    while(!published)
    {
        auto reserved = ++reservePosition_->reservePosition_;
        auto entryEnd = *readPosition_ + header_->entryCount_;
        while(entryEnd < reserved)
        {
            // todo: how can we get out of this?
        ...
        while(*publishPosition_ < reserved - 1)
        {
            std::this_thread::yield();
            std::atomic_thread_fence(std::memory_order::memory_order_acquire);
        }
        if(*publishPosition_  == reserved - 1)
        {
            *publishPosition_ = reserved;
        }
    }
