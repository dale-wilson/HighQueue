...
/// @brief HSQueue
///
/// An HSQueue is a data structure used for message passing between components of a system.  
///
/// Clients
///
/// The components that use an HSQueue will be called clients.  
///
...
///
/// Each instance of an HSQueue supports multiple Producers, but only a single Consumer.
///
/// Client API:
///
/// Clients interact with an HSQueue using an API that consists of the public interfaces of
/// four primary classes:   Message, Connection, Consumer, and Producer.
///
/// Two additonal classes are used when initializing an HSQueue: CreationParameters and ConsumerWaitStrategy.
///
/// The first step in using an HSQueue is to create an Connection using the null constructor.
///
/// This connection may be used to attach to an existing HSQueue or to create a new one using 
/// the methods: Connection::createLocal(), Connection::openOrCreateShared(), or Connection::openExistingShared(). 
/// 
/// The methods that create a new HSQueue take an instance of CreationParameters as an argument.
/// The CreationParameters in turn contains an ConsumerWaitStrategy.   See the documentation of these
/// classes for details about the parameters that can be used to configure the HSQueue for the particular use.
///
...
/// be initialized with memory the memory pool within the HSQueue.   The Connection::allocate(Message &)
...
/// producer using the same HSQueue they will be unaffected by the time it takes a particular producer
...
///
/// Users of HSQueues may stop reading here.   Developers and maintainers (and curious users) may continue
/// to:
///
/// Theory of Operation
///
...
///
/// In this implementation of an HSQueue addressing is always done via offsets to a base address rather than
/// actual addresses -- up to the point where final resolution into a physical address is needed.   This has an added benefit
/// of letting different clients in different processes work effectively with an HSQueue contained in shared memory 
...
/// the HSQueue.  Thus there is a pool of memory in the HSQueue itself that can be used to populate Messages.
///
/// Cache line alignment and related performance issues.
///
/// For performance reasons many of the objects contained in the HSQueue are aligned on cache line boundaries.
...
/// which must be visited by the garbage collector.   If the offset approach used in the HSQueue can be mapped 
...
///
/// This documentation is in HSQDefinitions.h
namespace HSQueue
{
