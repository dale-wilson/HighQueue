...
, reservePosition_(resolver_.resolve<volatile HSQReservePosition>(header_->reservePosition_)->reservePosition_)
, reserveSoloPosition_(resolver_.resolve<volatile HSQReservePosition>(header_->reservePosition_)->reserveSoloPosition_)
...
void Producer::publish(HSQueue::Message & message)
{
    ...
    while(!published)
    {
        ...
        HSQEntry & entry = entryAccessor_[reserved];
        if(entry.status_ != HSQEntry::Status::SKIP)
        {
            message.moveTo(entry.message_);
            entry.status_ = HSQEntry::Status::OK;
            published = true;
        }
