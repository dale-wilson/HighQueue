...
/// Client API:
///
/// Clients interact with an ProntoQueue using an API that consists of the public interfaces of
/// four primary classes:   Message, Connection, Consumer, and Producer.
///
/// Two additonal classes are used when initializing an ProntoQueue: CreationParameters and ConsumerWaitStrategy.
///
/// The first step in using an ProntoQueue is to create an Connection using the null constructor.
///
/// This connection may be used to attach to an existing ProntoQueue or to create a new one using 
/// the methods: Connection::createLocal(), Connection::openOrCreateShared(), or Connection::openExistingShared(). 
/// 
/// The methods that create a new ProntoQueue take an instance of CreationParameters as an argument.
/// The CreationParameters in turn contains an ConsumerWaitStrategy.   See the documentation of these
/// classes for details about the parameters that can be used to configure the ProntoQueue for the particular use.
///
...
/// producer using the same ProntoQueue they will be unaffected by the time it takes a particular producer
...
///
/// Users of ProntoQueues may stop reading here.   Developers and maintainers (and curious users) may continue
/// to:
///
/// Theory of Operation
///
...
///
/// In this implementation of an ProntoQueue addressing is always done via offsets to a base address rather than
/// actual addresses -- up to the point where final resolution into a physical address is needed.   This has an added benefit
/// of letting different clients in different processes work effectively with an ProntoQueue contained in shared memory 
...
/// the ProntoQueue.  Thus there is a pool of memory in the ProntoQueue itself that can be used to populate Messages.
///
/// Cache line alignment and related performance issues.
///
/// For performance reasons many of the objects contained in the ProntoQueue are aligned on cache line boundaries.
...
/// which must be visited by the garbage collector.   If the offset approach used in the ProntoQueue can be mapped 
